document.addEventListener('DOMContentLoaded', function () {
  ...
  const firebaseConfig = {
    apiKey: "AIzaSyDbgfpEdK4kZGRCH-dp7l2ZK8ChxmVj2fg",
    authDomain: "tigerfix-c0ef2.firebasestorage.app",
    databaseURL: "https://tigerfix-c0ef2-default-rtdb.europe-west1.firebasedatabase.app",
    projectId: "tigerfix-c0ef2",
    storageBucket: "tigerfix-c0ef2.firebasestorage.app",
    messagingSenderId: "855873438861",
    appId: "1:855873438861:android:c7e5555852ec2e4fe68ec4"
  };
...
const T_MAX_LEFT = 7.0;          // s (vizual animatsiya uchun)
const T_MAX_BOTTOM = 7.0;        // s
...
console.log("🕐 Ultra Slow Mode: 1.00x → 2.00x o‘sish 10 yil davom etadi!");    path.setAttribute('fill', 'none');
  ...
  function runRound() {
    ...
    if (!crashCoeff) {
        crashCoeff = (Math.random() * (24 - 21) + 21).toFixed(2);
    }
    
    const floatCrashCoeff = parseFloat(crashCoeff);
    nextRoundCrashCoeff = null;

    const roundId = 'round_' + Date.now();
    const roundPayload = { 
        id: roundId, 
        createdAt: Date.now(), 
        crashCoeff: floatCrashCoeff, 
        events: [] 
    };
...
const RATE_BASE = 0.07;
const RATE_STEP_1 = 0.09;
const RATE_STEP_2 = 0.09;
const RATE_AT_MAX_BOTTOM = 0.09; // Dastur aynan shu nomni qidirmoqda
const RATE_AT_MAX_BOTH = 0.27;
...
function animate(ts) {
    ...
    // Airplane joylashuvi — siz tahrirlaganingiz, tepaga 10px ko‘tarish
    const svgRect = svg.getBoundingClientRect();
    const pinWidth = pin.offsetWidth;
    const pinHeight = pin.offsetHeight;

    const px = ((x - clipRect.x) / clipRect.w) * svgRect.width + svgRect.width * 0.03;
    const py = ((y - clipRect.y) / clipRect.h) * svgRect.height;

    pin.style.left = `${px - pinWidth / 1}px`;
    pin.style.top = `${py - pinHeight / 2 - 14}px`; 
    pin.style.transform = 'rotate(-29deg)';
    ...
    // ✅ crashCircle: path oxirida harakat qiladi
    if (crashCircle) {
        crashCircle.setAttribute('cx', x.toFixed(2));
        crashCircle.setAttribute('cy', y.toFixed(2));
        crashCircle.setAttribute('r', '4');
        crashCircle.setAttribute('fill', '#de8a06');
        crashCircle.style.display = 'block';
    }

    // multiplikator hisoblash
    const crossedThresholds = (leftPct >= THRESH_LEFT_START) && (bottomPct >= THRESH_BOTTOM_START);
    let rate = 0;
    if (crossedThresholds) rate = RATE_BASE;
    if (bottomPct >= MAX_BOTTOM) rate = RATE_AT_MAX_BOTTOM;
    ...
    if (!crashed && score >= floatCrashCoeff) {
        crashed = true;
        pin.classList.add('crash-game__pin--crash'); // airplane crash effekt
